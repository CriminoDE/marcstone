...
// Driven purely by the server turn timer value passed in.
...
export function FuseTimer({ remaining, total = 30, active }: FuseTimerProps) {
  const pct = Math.max(0, Math.min(100, (remaining / total) * 100));
  const panic = remaining <= 5;
  ...
  return (
    <div className="w-full flex items-center gap-3 px-1 select-none" aria-hidden>
      ...
      <div className="relative flex-1 h-3">
        ...
        {/* Flame riding the burning tip */}
        <div
          className="absolute -top-[9px]"
          style={{ left: `calc(${pct}% - 9px)`, transition: "left 0.5s linear" }}
        >
          <span
            className={`block text-base leading-none ${panic ? "animate-pulse" : ""}`}
            style={{ filter: `drop-shadow(0 0 6px ${panic ? "#E03131" : "#FF5A2D"})` }}
          >
            🔥
          </span>
        </div>
      ...
      <span
        className={`font-display font-black text-sm tabular-nums ${
          panic ? "text-mg-danger animate-pulse" : active ? "text-mg-frost-text" : "text-mg-fog"
        }`}
      >
        {remaining}s
      </span>
